...
export class CommandsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  constructor(private readonly _service: CommandsService,
    ...
    this._commands[CommandClientToServer.COMMAND_EXPERIMENT_FINISH] = () => _service.finishExperiment(_experimentResults.activeExperimentResult?.id);
    this._commands[CommandClientToServer.COMMAND_EXPERIMENT_UPLOAD] = async (experimentId: number) => {
      await this._service.uploadExperiment(experimentId);
    };
    this._commands[CommandClientToServer.COMMAND_EXPERIMENT_SETUP] = async (experimentID: number) => {
