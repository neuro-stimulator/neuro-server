...
import {
  ...
  createEmptyExperimentERP, createEmptyExperimentResult,
  createEmptyOutputERP,
  createEmptySequence,
  Experiment,
  ExperimentERP, ExperimentResult, MessageCodes,
  Sequence,
...
describe('Commands service', () => {
  ...
  beforeEach(async () => {
    ...
    // MOCKS
    // @ts-ignore
    mockSerialService = testingModule.get<MockType<SerialService>>(SerialService);
    // @ts-ignore
    mockExperimentsService = testingModule.get<MockType<ExperimentsService>>(ExperimentsService);
    // @ts-ignore
    mockExperimentResultsService = testingModule.get<MockType<ExperimentResultsService>>(ExperimentResultsService);
    // @ts-ignore
    mockSequencesService = testingModule.get<MockType<SequencesService>>(SequencesService);
    // @ts-ignore
  ...
  describe('stimulatorState()', () => {
    ...
    it('positive - should send command for stimulator state and wait for the result', async () => {
      ...
      mockSerialService.bindEvent = jest.fn((name: string, callback: (event: EventStimulatorState) => void) => {
        return (
          setTimeout(() => {
  ...
  describe('uploadExperiment()', () => {
    it('positive - should upload experiment with sequence to stimulator', async () => {
      ...
      expect(mockSerialService.write).toBeCalledWith(bufferCommandEXPERIMENT_UPLOAD(experiment, sequence));
      expect(mockExperimentResultsService.createEmptyExperimentResult).toBeCalledWith(experiment);
    });
  ...
  describe('setupExperiment()', () => {
    it('positive - should send command for experiment initialization', async () => {
      ...
      Object.defineProperty(mockExperimentResultsService, 'activeExperimentResult', {
        get: jest.fn().mockReturnValue(experimentResult)
      });

      await commandsService.setupExperiment(experiment.id);

      expect(mockIpcService.send).toBeCalledWith(TOPIC_EXPERIMENT_STATUS, { status: 'setup', id: experiment.id });
      expect(mockSerialService.write).toBeCalledWith(bufferCommandMANAGE_EXPERIMENT('setup'));
    });
    ...
    it('negative - should not init not uploaded experiment', async (done: DoneCallback) => {
      ...
      try {
        ...
        expect(e.message).toBe(`${MessageCodes.CODE_ERROR_COMMANDS_EXPERIMENT_SETUP_NOT_UPLOADED}`);
  ...
  describe('runExperiment()', () => {
    it('positive - should send command for run experiment', async () => {
      ...
      expect(mockIpcService.send).toBeCalledWith(TOPIC_EXPERIMENT_STATUS, { status: 'run', id: experiment.id });
      expect(mockSerialService.write).toBeCalledWith(bufferCommandMANAGE_EXPERIMENT('run'));
    });
    ...
    it('negative - should not run not initialized experiment', async (done: DoneCallback) => {
      ...
      try {
        ...
        expect(e.message).toBe(`${MessageCodes.CODE_ERROR_COMMANDS_EXPERIMENT_RUN_NOT_INITIALIZED}`);
        expect(mockIpcService.send).not.toBeCalled();
        expect(mockSerialService.write).not.toBeCalled();
        done();
      }
  ...
  describe('pauseExperiment()', () => {
    it('positive - should send command for pause experiment', async () => {
      ...
      expect(mockIpcService.send).toBeCalledWith(TOPIC_EXPERIMENT_STATUS, { status: 'pause', id: experiment.id });
      expect(mockSerialService.write).toBeCalledWith(bufferCommandMANAGE_EXPERIMENT('pause'));
    });
    ...
    it('negative - should not pause not running experiment', async (done: DoneCallback) => {
      ...
      try {
        ...
        expect(e.message).toBe(`${MessageCodes.CODE_ERROR_COMMANDS_EXPERIMENT_PAUSE_NOT_STARTED}`);
        expect(mockIpcService.send).not.toBeCalled();
        expect(mockSerialService.write).not.toBeCalled();
        done();
      }
  ...
  describe('finishExperiment()', () => {
    it('positive - should send command for finish experiment', async () => {
      ...
      expect(mockIpcService.send).toBeCalledWith(TOPIC_EXPERIMENT_STATUS, { status: 'finish', id: experiment.id });
      expect(mockSerialService.write).toBeCalledWith(bufferCommandMANAGE_EXPERIMENT('finish'));
    });
    ...
    it('negative - should not finish not running experiment', async (done: DoneCallback) => {
      ...
      try {
        ...
        expect(e.message).toBe(`${MessageCodes.CODE_ERROR_COMMANDS_EXPERIMENT_FINISH_NOT_RUNNING}`);
        expect(mockIpcService.send).not.toBeCalled();
        expect(mockSerialService.write).not.toBeCalled();
        done();
      }
  ...
  describe('clearExperiment()', () => {
    it('positive - should send command for clear experiment', async () => {
      ...
      expect(mockIpcService.send).toBeCalledWith(TOPIC_EXPERIMENT_STATUS, { status: 'clear' });
      expect(mockSerialService.write).toBeCalledWith(bufferCommandMANAGE_EXPERIMENT('clear'));
    });
  });
  ...
  describe('sendNextSequencePart()', () => {
    it('positive - should send command with next sequence part', async () => {
      ...
      experiment.id = 1;
      experiment.outputCount = TOTAL_OUTPUT_COUNT;
      experiment.outputs = new Array(experiment.outputCount).fill(0).map((value, i: number) => createEmptyOutputERP(experiment, i));
      const experimentResult: ExperimentResult = createEmptyExperimentResult(experiment);
      const sequence: Sequence = createEmptySequence();
      sequence.size = 20;
      sequence.data = new Array(sequence.size).fill(0).map(() => Math.round(Math.random() * experiment.outputCount));
      const offset = 4;
      const index = 1;
      ...
      expect(mockSerialService.write).toBeCalledWith(bufferCommandNEXT_SEQUENCE_PART(sequence, offset, index));
    });
  });
});
