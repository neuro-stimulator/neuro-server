...
export class ExperimentResultsRepository {
  ...
  async one(id: number, userId: number): Promise<ExperimentResult> {
    const experimentResultEntity: ExperimentResultEntity = await this._repository.findOne({ where: { id, userId } });
