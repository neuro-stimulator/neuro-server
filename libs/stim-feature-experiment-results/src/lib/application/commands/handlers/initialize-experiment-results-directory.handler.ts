...
export class InitializeExperimentResultsDirectoryHandler
  implements
    ICommandHandler<InitializeExperimentResultsDirectoryCommand, void> {
  private readonly logger: Logger = new Logger(
    InitializeExperimentResultsDirectoryHandler.name
  );
  ...
  async execute(
    command: InitializeExperimentResultsDirectoryCommand
  ): Promise<void> {
    this.logger.debug('Budu inicializovat složku pro výsledky experimentů.');
    await this.facade.createNewFolder(
      `${ExperimentResultsService.EXPERIMENT_RESULTS_DIRECTORY_NAME}/${this.service.activeExperimentResult?.filename}`
    );
  }
}
