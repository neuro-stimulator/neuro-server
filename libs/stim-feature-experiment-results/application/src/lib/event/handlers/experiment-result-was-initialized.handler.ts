...
export class ExperimentResultWasInitializedHandler
  implements IEventHandler<ExperimentResultWasInitializedEvent> {
  private readonly logger: Logger = new Logger(
    ExperimentResultWasInitializedHandler.name
  );
  ...
  async handle(event: ExperimentResultWasInitializedEvent): Promise<any> {
    ...
    await this.commandBus.execute(
      new FillInitialIoDataCommand(event.timestamp)
    );
  }
}
