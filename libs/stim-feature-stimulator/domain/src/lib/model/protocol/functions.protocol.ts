...
export function bufferCommandDISPLAY_CLEAR(): Buffer {
  return Buffer.from(Uint8Array.from([CommandToStimulator.COMMAND_DISPLAY, CommandToStimulator.COMMAND_DISPLAY_ACTION_CLEAR, CommandToStimulator.COMMAND_DELIMITER]));
}
...
export function bufferCommandEXPERIMENT_UPLOAD(commandID: number = 0, experiment: Experiment, sequence?: Sequence): Buffer {
  ...
  serializedExperiment.experiment.writeUInt8(commandID, serializedExperiment.offset++);
  ...
  switch (experiment.type) {
    ...
    case ExperimentType.FVEP:
      serializer.serializeExperimentFVEP(commandID, experiment as ExperimentFVEP, serializedExperiment);
      break;
    case ExperimentType.TVEP:
      serializer.serializeExperimentTVEP(commandID, experiment as ExperimentTVEP, serializedExperiment);
      break;
...
export function bufferMemory(memoryType: number): Buffer {
  return Buffer.from(Uint8Array.from([CommandToStimulator.COMMAND_MEMORY, memoryType, CommandToStimulator.COMMAND_DELIMITER]));
}
