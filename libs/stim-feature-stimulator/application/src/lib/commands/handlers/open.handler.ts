...
import { SerialPortOpenSettings } from '@neuro-server/stim-feature-stimulator/domain';
...
export class OpenHandler implements ICommandHandler<OpenCommand> {
  ...
  async execute(command: OpenCommand): Promise<void> {
    ...
    const serialConfig: SerialPortOpenSettings = settings.serial as unknown as SerialPortOpenSettings;
