...
export class ExperimentErpRepository
  ...
  private readonly _erpOutputDepRepository: Repository<
  ...
  constructor(private readonly _manager: EntityManager) {
    ...
    this._erpOutputRepository = _manager.getRepository(
      ExperimentErpOutputEntity
    );
    this._erpOutputDepRepository = _manager.getRepository(
      ExperimentErpOutputDependencyEntity
    );
  }

  private async _updateOutputDependencies(
    repository: Repository<ExperimentErpOutputDependencyEntity>,
    output: ErpOutput
  ): Promise<any> {
    ...
    const intersection = outputDependencies.filter(
      (value: OutputDependency) => {
        return (
          databaseDependencies.findIndex(
            (dependency: ExperimentErpOutputDependencyEntity) =>
    ...
    const deleted = databaseDependencies.filter(
      (value: ExperimentErpOutputDependencyEntity) => {
        return !(
          outputDependencies.findIndex(
            (dependency: OutputDependency) => value.id === dependency.id
          ) > -1
        );
      }
    );
    ...
    if (deleted.length > 0) {
      this.logger.verbose(
        `Mažu závislosti: ${deleted.map((value) => value.id).join(', ')}`
      );
    ...
    for (const outputDependency of intersection) {
      ...
      this.logger.verbose(
        experimentErpOutputDependencyToEntity(outputDependency)
      );
  ...
  async update(experiment: ExperimentERP): Promise<any> {
    await this._manager.transaction(async (transactionManager) => {
      const erpRepository = transactionManager.getRepository(
        ExperimentErpEntity
      );
      const erpOutputRepository = transactionManager.getRepository(
        ExperimentErpOutputEntity
      );
      const erpOutputDepRepository = transactionManager.getRepository(
        ExperimentErpOutputDependencyEntity
      );
      ...
      for (const output of experiment.outputs) {
        ...
        await erpOutputRepository.update(
          { id: output.id },
      ...
      await erpRepository.update(
        { id: experiment.id },
        experimentErpToEntity(experiment)
      );
    });
  }
