...
import { ExperimentPlayerDataIOMessage } from '@stechy1/diplomka-share';
...
export class SendStimulatorIoDataToClientHandler implements ICommandHandler<SendStimulatorIoDataToClientCommand, void> {
  private readonly logger: Logger = new Logger(SendStimulatorIoDataToClientHandler.name);
  ...
  async execute(command: SendStimulatorIoDataToClientCommand): Promise<void> {
    ...
    await this.facade.broadcastCommand(new ExperimentPlayerDataIOMessage(command.ioEvent));
  }
}
