...
import { FILE_BROWSER_BASE_PATH } from 'apps/server/src/app/config/config';
...
export class FileBrowserService {
  ...
  constructor() {
    this.logger.log(
      `Základní cesta ke všem souborům je: '${FileBrowserService.BASE_PATH}'.`
    );
  ...
  public mergePrivatePath(...subfolders: string[]) {
    return path.join(
      FileBrowserService.PRIVATE_PATH,
    ...
    try {
      ...
      return files.map((file: string) => {
        ...
        return {
          name: file,
          path: fullPath
            .replace(`${FileBrowserService.PUBLIC_PATH}${path.sep}`, '')
            .replace('\\', '/'),
          isDirectory,
