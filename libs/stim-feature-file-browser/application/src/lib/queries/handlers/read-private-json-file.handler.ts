...
export class ReadPrivateJSONFileHandler implements IQueryHandler<ReadPrivateJSONFileQuery, string> {
  ...
  async execute(query: ReadPrivateJSONFileQuery): Promise<string> {
    this.logger.debug(`Budu číst privátní JSON soubor na cestě: '${query.path}'.`);
    const path = this.service.mergePrivatePath(true, query.path);
    return JSON.parse(this.service.readFileBuffer(path, { encoding: 'utf-8' }) as string);
  }
}
