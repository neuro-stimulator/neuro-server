...
import { createSerialServiceMock } from '../../src/app/low-level/serial.service.jest';
...
describe('Experiments integration test', () => {
  ...
  beforeEach(async () => {
    const module = await Test.createTestingModule({
      imports: [createInMemoryTypeOrmModule(), ExperimentsModule],
      providers: [
        ExperimentsService,
  ...
  describe('all()', () => {
    it('positive - should return all existing experiments', async () => {
      const experiments: Experiment[] = new Array(1)
        .fill(0)
  ...
  describe('nameExists()', () => {
    ...
    it('negative - should check name existence of existing experiment', async () => {
      ...
      return request(httpServer)
        .get(
          `${BASE_API}/name-exists/${staticExperiment.name}/${experiment.id}`
        )
        .expect(200)
        .expect(expected);
    });
  ...
  describe('insert()', () => {
    describe('ERP experiment', () => {
      it('positive - should insert new experiment', async () => {
        ...
        expectedExperiment.outputs = new Array<number>(
          expectedExperiment.outputCount
        )
          .fill(0)
          .map((value: number, index: number) => {
            const output: ErpOutput = createEmptyOutputERP(
              expectedExperiment,
              index
            );
            output.id = index + 1;
    ...
    describe('FVEP experiment', () => {
      it('positive - should insert new experiment', async () => {
        ...
        expectedExperiment.outputs = new Array<number>(
          ...
          .fill(0)
          .map((value: number, index: number) => {
            const output: FvepOutput = createEmptyOutputFVEP(
              expectedExperiment,
              index
            );
            output.id = index + 1;
    ...
    describe('TVEP experiment', () => {
      it('positive - should insert new experiment', async () => {
        ...
        expectedExperiment.outputs = new Array<number>(
          ...
          .fill(0)
          .map((value: number, index: number) => {
            const output: TvepOutput = createEmptyOutputTVEP(
              expectedExperiment,
              index
            );
            output.id = index + 1;
