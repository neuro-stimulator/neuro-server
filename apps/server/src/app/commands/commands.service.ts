...
export class CommandsService implements MessagePublisher {
  ...
  constructor(
    ...
    this._serial.bindEvent(EventNextSequencePart.name, (event) =>
      this._sendNextSequencePart(event)
    );
  }
  ...
  public async stimulatorState(
  ...
  ): Promise<EventStimulatorState | undefined> {
    ...
    return new Promise((resolve, reject) => {
      ...
      function serialEventCallback(event: EventStimulatorState | undefined) {
        ...
        self._serial.unbindEvent(
          EventStimulatorState.name,
          serialEventCallback
        );
