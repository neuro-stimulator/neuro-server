...
import { createEmptyExperimentCVEP, createEmptyExperimentResult, Experiment, ExperimentResult, MessageCodes, ResponseObject } from '@stechy1/diplomka-share';
...
import { createExperimentResultsServiceMock } from "./experiment-results.service.jest";
...
describe('Experiment results controller', () => {
  ...
  describe('experimentResultById()', () => {
    ...
    it('negative - should return error code when experiment result not found', async (done: DoneCallback) => {
      ...
      await controller.experimentResultById({ id: 1 })
                      .then(() => {
                        done.fail();
                      })
  ...
  describe('resultData()', () => {
    it('positive - should return result data', async () => {
      ...
      const result: ResponseObject<any> = await controller.resultData({ id: 1 });
    ...
    it('negative - should return error code when experiment result not found', async (done: DoneCallback) => {
      ...
      await controller.resultData({ id: 1 })
                      .then(() => {
                        done.fail();
                      })
                      .catch((exception: ControllerException) => {
                        expect(exception.code).toEqual(MessageCodes.CODE_ERROR_EXPERIMENT_RESULT_DATA_NOT_FOUND);
                        expect(exception.params).toEqual({ id: 1 });
                        done();
                      });
    });
  ...
  describe('update()', () => {
    ...
    it('negative - should return error code when experiment result not found', async (done: DoneCallback) => {
      ...
      await controller.update(experimentResult)
                      .then(() => {
                        done.fail();
                      })
                      .catch((exception: ControllerException) => {
  ...
  describe('delete()', () => {
    it('positive - should delete experiment result', async () => {
      ...
      const result: ResponseObject<ExperimentResult> = await controller.delete({ id: experimentResult.id });
      const expected: ResponseObject<ExperimentResult> = {
        data: experimentResult,
        message: {
    ...
    it('negative - should return error code when experiment result not found', async (done: DoneCallback) => {
      ...
      await controller.delete({ id: experimentResult.id })
                      .then(() => {
                        done.fail();
                      })
                      .catch((exception: ControllerException) => {
