...
export class FileBrowserController {
  ...
  @Get('*')
  public async getContentContent(
    @Param() param: string[],
  ...
  public async createNewFolder(
    ...
    try {
      const subfolderPath = this._service.mergePublicPath(...subfolders);
      const originalSubfolderPath = await this._service.mergePublicPath(
        ...originalSubfolders
      );
      await this._service.createDirectory(subfolderPath, true);
      const files = await this._service.getFilesFromDirectory(
        originalSubfolderPath
      );
