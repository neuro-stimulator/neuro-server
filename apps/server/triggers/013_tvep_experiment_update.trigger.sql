CREATE TRIGGER IF NOT EXISTS tvep_experiment_update AFTER UPDATE
    ON experiment_tvep_entity
    WHEN (SELECT enabled FROM trigger_control WHERE trigger_control.name = 'tvep_experiment_update')=1
BEGIN
